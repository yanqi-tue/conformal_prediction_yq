...
#         fgr[i]=0
LGA90_ARIS_relabel0=[]
LGA90_ARIS_ID_relabel0=[]
LGA90_ARIS_index=[]
for i in range(len(LGA90_ARIS)):
    if LGA90_ARIS[i] == 1:
        LGA90_ARIS_relabel0.append(1.0)
        LGA90_ARIS_ID_relabel0.append(GDM_ID[i])
        LGA90_ARIS_index.append(i)
    if LGA90_ARIS[i] == 0:
        LGA90_ARIS_relabel0.append(0.0)
        LGA90_ARIS_ID_relabel0.append(GDM_ID[i])
        LGA90_ARIS_index.append(i)
...
OUTPUT_DIR = os.path.join(
    RESULTS_ROOT,
    "LGA90_LR_nestedCV_performance_metrics",
)
...
label1 = np.asarray(LGA90_ARIS_relabel0, dtype=int)
master_index = np.asarray(LGA90_ARIS_index, dtype=int)
subject_ids_master = np.asarray(LGA90_ARIS_ID_relabel0, dtype=object)
...
if len(label1) != len(master_index):
    raise ValueError(
        "The LGA90 label length does not match LGA90_ARIS_index: "
        f"{len(label1)} versus {len(master_index)}."
    )
if len(subject_ids_master) != len(label1):
    raise ValueError(
        "LGA90_ARIS_ID_relabel0 does not align with the LGA90 label vector: "
...
print(
    "LGA90 events among pregnancies delivered before U3:",
...
def run_visit_model(visit_name, feature_names, cv_splits, analysis_mask):
    ...
    for model_name, full_predictions in oof_predictions_by_model.items():
        ...
        print(
            f"{visit_name} {model_name} LGA90 AUC = {auc_value:.4f} "
...
def summarize_state_prevalence(trajectory_df):
    ...
    subsets = {
        ...
        "LGA90-positive only": trajectory_df["LGA90"] == 1,
        "LGA90-negative only": trajectory_df["LGA90"] == 0,
    }
...
def summarize_empirical_coverage(trajectory_df, confidence):
    ...
    for visit_name in ["U1", "U2", "U3"]:
        ...
        y = trajectory_df.loc[at_risk, "LGA90"].astype(int)
        covered = (
    ...
    outcome_summary = pd.DataFrame(
        {
            ...
            "N": [len(early)],
            "LGA90 events": [int(early["LGA90"].sum())],
            "LGA90 event rate": [float(early["LGA90"].mean())],
        }
    )
    path_summary = (
        early.groupby(["LGA90", "U1_U2_trajectory"], dropna=False)
        .size()
        .reset_index(name="N")
        .sort_values(["LGA90", "N"], ascending=[False, False])
        ...
        rows.append(
            {
                ...
                "LGA90 events": event_n,
                "LGA90 event rate": event_rate,
...
def build_aligned_co_outcomes():
    """Build descriptive co-outcomes aligned to the LGA90 master cohort."""
...
def summarize_co_outcome_enrichment(trajectory_df, co_outcomes):
    ...
    group_masks = {
        "Whole LGA90 modelling cohort": pd.Series(True, index=trajectory_df.index),
...
def print_trajectory_analysis(trajectory_df, confidence, co_outcomes=None):
    print("\n" + "=" * 100)
    print(
        f"SUBJECT-LEVEL LGA90 TRAJECTORY ANALYSIS: confidence={confidence:.0%}, "
        f"alpha={1.0-confidence:.2f}"
    )
    print("=" * 100)
    eligible = trajectory_df.loc[trajectory_df["U3_landmark_eligible"]]
    print(
        f"Master cohort N={len(trajectory_df)}, LGA90 events={int(trajectory_df['LGA90'].sum())} "
        f"({trajectory_df['LGA90'].mean():.2%}); U3 risk set N={len(eligible)}, "
        f"events={int(eligible['LGA90'].sum())} ({eligible['LGA90'].mean():.2%}); "
    ...
    print("\n4) Exact trajectory distribution among LGA90-positive pregnancies in the U3 risk set:")
    ...
    print("\n5) Clinically grouped trajectories and observed LGA90 incidence:")
    print(grouped_summary.to_string(index=False, float_format=lambda x: f"{x:.4f}"))

    transition_tables = {}
    for positive_only in [False, True]:
        subset_label = "LGA90-positive only" if positive_only else "all samples"
        for from_visit, to_visit in [("U1", "U2"), ("U2", "U3")]:
...
OUTCOME_NAME_FOR_ALIGNMENT = "LGA90"


def _full_cohort_exclusion_reason():
    birth_weight = np.asarray(bw, dtype=float)
    ga_week = np.asarray(GA_WK, dtype=float)
    fetal_sex = np.asarray(SEX, dtype=float)
    lga = np.asarray(LGA90_ARIS, dtype=float)

    reason = np.full(FULL_COHORT_N, "", dtype=object)
    excluded = ~np.isin(lga, [0.0, 1.0])

    reason[excluded & ~np.isfinite(birth_weight)] = "missing_birth_weight"
    reason[excluded & np.isfinite(birth_weight) & (birth_weight <= 0.0)] = "invalid_birth_weight"

    unset = excluded & (reason == "")
    reason[unset & ~np.isfinite(ga_week)] = "missing_delivery_GA"
    unset = excluded & (reason == "")
    reason[
        unset
        & (
    ...
    ] = "delivery_GA_outside_Aris_reference"

    unset = excluded & (reason == "")
    reason[unset & ~np.isfinite(fetal_sex)] = "missing_fetal_sex"
    unset = excluded & (reason == "")
    reason[unset & ~np.isin(fetal_sex, [MALE_CODE, FEMALE_CODE])] = (
        "unsupported_fetal_sex_code"
    )

    reason[excluded & (reason == "")] = "LGA90_outcome_not_defined"
    return reason
...
def save_analysis_results(
    ...
    excel_path = os.path.join(OUTPUT_DIR, f"LGA90_{MODEL_SELECTION}_CP_results.xlsx")
    ...
    with open(config_path, "w", encoding="utf-8") as handle:
        handle.write("Outcome: LGA90 (Aris 2017 first-born reference)\n")
...
def run_performance_visit(visit_name, feature_names, cv_splits, analysis_mask):
    ...
    summary = {
        "outcome": "LGA90", "visit": visit_name, "n": int(len(y)), "events": int(y.sum()),
...
summary_path = os.path.join(OUTPUT_DIR, "LGA90_nestedCV_performance_summary.csv")
subject_path = os.path.join(OUTPUT_DIR, "LGA90_nestedCV_subject_level_oof_predictions.csv")
tuning_path = os.path.join(OUTPUT_DIR, "LGA90_nestedCV_inner_tuning_all_candidates.csv")
selected_path = os.path.join(OUTPUT_DIR, "LGA90_nestedCV_selected_hyperparameters_by_fold.csv")
