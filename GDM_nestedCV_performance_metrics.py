...
# Raw oDM phenotype used to define the GDM modelling target.
#   oDM = 1: pre-existing diabetes mellitus (excluded)
#   oDM = 2: gestational diabetes mellitus (target class 1)
#   oDM = 3: no gestational diabetes mellitus (target class 0)
#   missing/other: excluded
odm_raw = np.asarray(
    data_read_and_fill_fornum(40, 'oDM', GDM_ID),
    dtype=float,
)
...
# =============================================================================
# GDM outcome used by all downstream modelling and trajectory analyses
# =============================================================================
# Only women with an explicit oDM code of 2 or 3 enter the modelling cohort.
# Pre-existing diabetes (oDM=1), missing oDM, and unsupported codes are excluded
# before cross-validation, preprocessing, model fitting, conformal calibration,
# coverage estimation, and trajectory analysis.
GDM_relabel0 = []
GDM_ID_relabel0 = []
GDM_index = []

for i, odm_code in enumerate(odm_raw):
    if not np.isfinite(odm_code):
        continue
    if odm_code == 2.0:
        GDM_relabel0.append(1.0)
        GDM_ID_relabel0.append(GDM_ID[i])
...
GDM_relabel0 = np.asarray(GDM_relabel0, dtype=int)
GDM_ID_relabel0 = np.asarray(GDM_ID_relabel0, dtype=object)
GDM_index = np.asarray(GDM_index, dtype=int)

GDM_DM_EXCLUDED_N = int(np.sum(np.isfinite(odm_raw) & (odm_raw == 1.0)))
GDM_MISSING_EXCLUDED_N = int(np.sum(~np.isfinite(odm_raw)))
GDM_OTHER_EXCLUDED_N = int(
    np.sum(np.isfinite(odm_raw) & ~np.isin(odm_raw, [1.0, 2.0, 3.0]))
)

if len(GDM_relabel0) == 0:
    raise ValueError('No eligible oDM=2/3 records were found for GDM modelling.')
if np.unique(GDM_relabel0).size != 2:
    raise ValueError(
        'The eligible GDM cohort must contain both oDM=2 and oDM=3 classes.'
    )

print(
    'GDM outcome (oDM=2 versus oDM=3):',
    int(GDM_relabel0.sum()),
    '/',
    len(GDM_relabel0),
    '=',
    round(float(GDM_relabel0.mean()) * 100.0, 2),
    '%',
)
print(
    'Excluded from GDM modelling: oDM=1 (pre-existing DM)=',
    GDM_DM_EXCLUDED_N,
    '; missing oDM=',
    GDM_MISSING_EXCLUDED_N,
    '; unsupported oDM codes=',
    GDM_OTHER_EXCLUDED_N,
)

# Co-outcome source used later in build_aligned_co_outcomes().
# CMAE08 coding follows the existing FGR analysis: 1 = FGR, 2 = no FGR.
fgr = np.asarray(
    data_read_and_fill_fornum(26, 'CMAE08', GDM_ID),
    dtype=float,
)
...
# Preserve the original feature-name lists before adding GDM-specific U2 features.
# These snapshots must be created before the later cumulative feature updates.
...
# Prevent direct target leakage. The derived variables named `GDM` and
# `pre-existed DM` are present in the historical full feature matrix but must
# not be used to predict the oDM-based GDM target.
GDM_TARGET_LEAKAGE_FEATURES = {"GDM", "pre-existed DM"}
for _visit_name, _feature_names in BASE_FEATURE_SETS.items():
    _leakage = sorted(GDM_TARGET_LEAKAGE_FEATURES.intersection(_feature_names))
    if _leakage:
        raise ValueError(
            f"{_visit_name} contains target-leakage features: {_leakage}."
        )
# ============================================================================
# Helpers for loading visit-specific numeric variables
# ============================================================================
...
def get_dataset_index(dataset_identifier):
    """
    Return the index of a nuMoM2b dataset in ``dataset_name``.

    Matching is case-insensitive, so identifiers such as ``v1b`` and ``V1B``
    are treated as the same dataset. A clear error is raised when the dataset
    cannot be found instead of silently using an incorrect index.
    """
    target = str(dataset_identifier).strip().upper()

    for dataset_index, current_name in enumerate(dataset_name):
        if str(current_name).strip().upper() == target:
            return dataset_index

    available = ", ".join(str(name) for name in dataset_name)
    raise KeyError(
        f"Dataset '{dataset_identifier}' was not found. "
        f"Available datasets are: {available}"
    )


def load_numeric(dataset_identifier, variable_name, label_ids=None):
    """
    Load one numeric nuMoM2b variable aligned to the requested subject IDs.

    Parameters
    ----------
    dataset_identifier : str
        Dataset name, for example ``'v1b'`` or ``'v2b'``.
    variable_name : str
        Variable name in the nuMoM2b data dictionary.
    label_ids : sequence, optional
        Subject IDs defining the output order. By default, the complete
        ``GDM_ID`` order is used so the returned array aligns with the main
        feature matrix before application of ``master_index``.

    Returns
    -------
    np.ndarray
        One-dimensional float array. Missing values are represented by NaN.
    """
    ...
    variable_lookup = {
        str(name).strip().upper(): str(name)
        for name in full_variable_name[dataset_index]
    }
    target_variable = str(variable_name).strip().upper()

    if target_variable not in variable_lookup:
        raise KeyError(
            f"Variable '{variable_name}' was not found in dataset "
            f"'{dataset_name[dataset_index]}'."
        )

    canonical_variable_name = variable_lookup[target_variable]
    values = data_read_and_fill_fornum(
        dataset_index,
    ...
    if values.size != len(label_ids):
        raise ValueError(
            f"Loaded variable '{canonical_variable_name}' from dataset "
            f"'{dataset_name[dataset_index]}' has {values.size} values, "
            f"but {len(label_ids)} subject IDs were requested."
        )
...
# ============================================================================
# GDM-specific features added at U2
#
# The helper functions above load all added variables in the original GDM_ID
# order. The oDM-based eligibility index is applied later through master_index,
# so oDM=1, missing oDM, and unsupported oDM codes remain excluded from every
# downstream model and trajectory analysis.
# ============================================================================
...
def mean_available_measurements(*arrays):
    """
    Calculate the row-wise mean across available measurements.
    If all measurements are missing, return NaN.
    """
    stacked = np.vstack([
    ...
    output[valid] = (
        np.nansum(stacked[:, valid], axis=0) /
        valid_count[valid]
    )
...
maternal_weight_visit_1 = load_numeric(
    'v1b',
    'V1BA01_KG'
)

maternal_weight_visit_2 = load_numeric(
    'v2b',
    'V2BA01_KG'
)

visit_1_date_int = load_numeric(
    'v1b',
    'V1BDATE_INT'
)

visit_2_date_int = load_numeric(
    'v2b',
    'V2BDATE_INT'
)


# VisitDate_INT均相对于project EDD，
# 因此两者相减可得到两次访视间真实的天数间隔
interval_days_visit_1_to_2 = (
    visit_2_date_int -
    visit_1_date_int
)

interval_weeks_visit_1_to_2 = (
    interval_days_visit_1_to_2 / 7.0
)
...
gestational_weight_gain_visit_1_to_2 = (
    maternal_weight_visit_2 -
    maternal_weight_visit_1
)

invalid_weight_gain = (
    ~np.isfinite(maternal_weight_visit_1) |
    ~np.isfinite(maternal_weight_visit_2)
)
...
gestational_weight_gain_rate_visit_1_to_2 = np.full(
    N_SAMPLE,
    np.nan,
    dtype=float
)

valid_weight_gain_rate = (
    np.isfinite(gestational_weight_gain_visit_1_to_2) &
    np.isfinite(interval_weeks_visit_1_to_2) &
    (interval_weeks_visit_1_to_2 > 0)
)

gestational_weight_gain_rate_visit_1_to_2[
    valid_weight_gain_rate
] = (
    gestational_weight_gain_visit_1_to_2[
        valid_weight_gain_rate
...
# ============================================================================
# 4. V1身高、V1 BMI、V2 BMI及BMI变化
# ============================================================================

maternal_height_visit_1 = mean_available_measurements(
    load_numeric('v1b', 'V1BA02a'),
    load_numeric('v1b', 'V1BA02b'),
    load_numeric('v1b', 'V1BA02c')
)

maternal_height_m = (
    maternal_height_visit_1 / 100.0
)


bmi_visit_1_calculated = np.full(
    N_SAMPLE,
    np.nan,
    dtype=float
)

bmi_visit_2_calculated = np.full(
    N_SAMPLE,
    np.nan,
    dtype=float
)
...
valid_bmi_visit_1 = (
    valid_height &
    np.isfinite(maternal_weight_visit_1) &
    (maternal_weight_visit_1 > 0)
)

valid_bmi_visit_2 = (
    valid_height &
    np.isfinite(maternal_weight_visit_2) &
    (maternal_weight_visit_2 > 0)
)

bmi_visit_1_calculated[
    valid_bmi_visit_1
] = (
    maternal_weight_visit_1[
        valid_bmi_visit_1
...
bmi_change_visit_1_to_2 = (
    bmi_visit_2_calculated -
    bmi_visit_1_calculated
)

invalid_bmi_change = (
    ~np.isfinite(bmi_visit_1_calculated) |
    ~np.isfinite(bmi_visit_2_calculated)
)
...
sbp_visit_1 = mean_available_measurements(
    load_numeric('v1b', 'V1BA06a1'),
    load_numeric('v1b', 'V1BA06a2')
)

dbp_visit_1 = mean_available_measurements(
    load_numeric('v1b', 'V1BA06b1'),
    load_numeric('v1b', 'V1BA06b2')
)

sbp_visit_2 = mean_available_measurements(
    load_numeric('v2b', 'V2BA02a1'),
    load_numeric('v2b', 'V2BA02a2')
)

dbp_visit_2 = mean_available_measurements(
    load_numeric('v2b', 'V2BA02b1'),
    load_numeric('v2b', 'V2BA02b2')
)
...
map_change_visit_1_to_2 = (
    map_visit_2 -
    map_visit_1
)

invalid_map_change = (
    ~np.isfinite(map_visit_1) |
    ~np.isfinite(map_visit_2)
)
...
head_circumference_visit_2 = load_numeric(
    'u2a',
    'U2AB03'
)

abdominal_circumference_visit_2 = load_numeric(
    'u2a',
    'U2AB04'
)


ac_hc_ratio_visit_2 = np.full(
    N_SAMPLE,
    np.nan,
    dtype=float
)
...
valid_ac_hc = (
    np.isfinite(abdominal_circumference_visit_2) &
    np.isfinite(head_circumference_visit_2) &
    (head_circumference_visit_2 > 0)
)

ac_hc_ratio_visit_2[
    valid_ac_hc
] = (
    abdominal_circumference_visit_2[
        valid_ac_hc
    ] /
    head_circumference_visit_2[
        valid_ac_hc
    ]
)
...
# 7. V2体力活动接口
#
# 完成physical_activity长格式数据汇总后，将下面的None替换为：
#
# physical_activity_visit_2 = 你的V2总MET-min/week数组
# physical_activity_change_visit_1_to_2 = V2减去V1的MET-min/week数组
#
# 两个数组长度都必须等于len(GDM_ID)。
# ============================================================================

physical_activity_visit_2 = None
physical_activity_change_visit_1_to_2 = None
...
GDM_add_U2 = [
    'Gestational weight gain from visit 1 to visit 2',
    'Gestational weight gain rate from visit 1 to visit 2',
    'BMI change from visit 1 to visit 2',
    'AC-to-HC ratio at visit 2',
    'MAP change from visit 1 to visit 2'
]
...
# ============================================================================
# 9. 如果已经完成体力活动汇总，则自动加入
# ============================================================================

if physical_activity_visit_2 is not None:

    physical_activity_visit_2 = np.asarray(
        physical_activity_visit_2,
        dtype=float
    ).reshape(-1)

    if len(physical_activity_visit_2) != N_SAMPLE:
        raise ValueError(
            'physical_activity_visit_2 does not align with GDM_ID.'
        )

    GDM_add_U2.append(
        'Physical activity at visit 2'
    )
...
if physical_activity_change_visit_1_to_2 is not None:

    physical_activity_change_visit_1_to_2 = np.asarray(
        physical_activity_change_visit_1_to_2,
        dtype=float
    ).reshape(-1)

    if len(physical_activity_change_visit_1_to_2) != N_SAMPLE:
        raise ValueError(
            'physical_activity_change_visit_1_to_2 '
            'does not align with GDM_ID.'
        )

    GDM_add_U2.append(
        'Change in physical activity from visit 1 to visit 2'
    )
...
factor_add_U2 = pd.DataFrame(
    factor_add_U2_dict
)[GDM_add_U2]


# GDM在U3前后已经进入常规筛查阶段，
# 因此不再增加新的U3特异变量，但U3模型可继续使用U2新增信息
factor_add_U3 = factor_add_U2.copy()
factor_add = factor_add_U2.copy()
...
if len(GDM_ID) != len(factor_add_U2):
    raise ValueError(
        'The GDM-specific U2 feature dataframe does not align with GDM_ID: '
        f'{len(factor_add_U2)} versus {len(GDM_ID)} subjects.'
    )
...
print(
    '\nMissing values in GDM U2 added features:'
)

print(
    factor_add_U2.isna().sum()
)


factor_add_U2.to_csv(
    os.path.join(INTERMEDIATE_OUTPUT_DIR, "factor_add_GDM_U2.csv"),
    index=True
)

factor_add.to_pickle(
    os.path.join(INTERMEDIATE_OUTPUT_DIR, "factor_add_GDM_U2.pkl")
)
...
factor_add_U3_np = factor_add_U2_np.copy()
factor_add_np = factor_add_U2_np.copy()
...
# ============================================================================
# 13. Merge GDM-specific U2 features into factor_all_num
# ============================================================================

factor_all_num_base = np.asarray(
    factor_all_num
)
...
if factor_all_num_base.ndim != 2:
    raise ValueError(
        'factor_all_num must be a two-dimensional '
        'feature-by-subject matrix.'
    )

if (
    factor_all_num_base.shape[1] !=
    factor_add_all_visits_np.shape[1]
):
    raise ValueError(
        'The original and GDM-specific matrices contain different '
        f'numbers of subjects: {factor_all_num_base.shape[1]} versus '
        f'{factor_add_all_visits_np.shape[1]}.'
    )

if factor_add_all_visits_np.shape[0] != len(GDM_add_U2):
    raise ValueError(
        'The number of added feature rows does not match GDM_add_U2.'
    )

if list(factor_add_U2.columns) != list(GDM_add_U2):
    raise ValueError(
        'factor_add_U2 columns do not follow GDM_add_U2 order.'
    )


overlapping_added_names = sorted(
    set(Name_explain_full_base).intersection(GDM_add_U2)
)

if overlapping_added_names:
    raise ValueError(
        'The following GDM-specific feature names already exist in '
        'the original feature matrix: ' +
        ', '.join(overlapping_added_names)
    )


# U1不增加特征
Name_explainx_U1 = list(
    Name_explainx_U1_base
)

# U2加入新增特征
Name_explainx_U2 = (
    list(Name_explainx_U2_base) +
    list(GDM_add_U2)
)

# U3继续使用U2阶段已经获得的新增特征
Name_explainx_U3 = (
    list(Name_explainx_U3_base) +
    list(GDM_add_U2)
)


Name_explain_full = (
    list(Name_explain_full_base) +
    list(GDM_add_U2)
)

# The modelling loop must use the cumulative visit-specific feature sets,
# including the GDM-specific U2 features at U2 and U3.
FEATURE_SETS = {
    "U1": Name_explainx_U1,
    "U2": Name_explainx_U2,
    "U3": Name_explainx_U3,
...
for _visit_name, _feature_names in FEATURE_SETS.items():
    _leakage = sorted(GDM_TARGET_LEAKAGE_FEATURES.intersection(_feature_names))
    if _leakage:
        raise ValueError(
            f"{_visit_name} contains target-leakage features: {_leakage}."
...
OUTPUT_DIR = os.path.join(
    RESULTS_ROOT,
    "GDM_LR_nestedCV_performance_metrics",
)
...
label1 = np.asarray(GDM_relabel0, dtype=int)
master_index = np.asarray(GDM_index, dtype=int)
subject_ids_master = np.asarray(GDM_ID_relabel0, dtype=object)
...
if len(label1) != len(master_index):
    raise ValueError(
        "The oDM-derived GDM label length does not match GDM_index: "
        f"{len(label1)} versus {len(master_index)}."
    )
if len(subject_ids_master) != len(label1):
    raise ValueError(
        "GDM_ID_relabel0 does not align with the oDM-derived GDM label vector: "
...
GA_WK_aligned = np.asarray(GA_WK, dtype=float)[master_index]
GA_DAYS_aligned = np.asarray(GA_DAYS, dtype=float)[master_index]
...
print(
    "GDM events among pregnancies delivered before U3:",
...
def run_visit_model(visit_name, feature_names, cv_splits, analysis_mask):
    ...
    for model_name, full_predictions in oof_predictions_by_model.items():
        ...
        print(
            f"{visit_name} {model_name} GDM AUC = {auc_value:.4f} "
    ...
    trajectory_df = pd.DataFrame(
        {
            ...
            "GDM": label1.astype(int),
            "oDM_raw_code": odm_raw[master_index],
...
def summarize_state_prevalence(trajectory_df):
    ...
    subsets = {
        ...
        "GDM-positive only": trajectory_df["GDM"] == 1,
        "GDM-negative only": trajectory_df["GDM"] == 0,
    }
...
def summarize_empirical_coverage(trajectory_df, confidence):
    ...
    for visit_name in ["U1", "U2", "U3"]:
        ...
        y = trajectory_df.loc[at_risk, "GDM"].astype(int)
        covered = (
    ...
    outcome_summary = pd.DataFrame(
        {
            ...
            "N": [len(early)],
            "GDM events": [int(early["GDM"].sum())],
            "GDM event rate": [float(early["GDM"].mean())],
        }
    )
    path_summary = (
        early.groupby(["GDM", "U1_U2_trajectory"], dropna=False)
        .size()
        .reset_index(name="N")
        .sort_values(["GDM", "N"], ascending=[False, False])
...
def build_aligned_co_outcomes():
    """Build descriptive non-GDM outcomes aligned to the GDM master cohort."""
    ...
    for outcome_name, values in co_outcomes.items():
        if len(values) != len(label1):
            raise ValueError(
                f"Co-outcome {outcome_name} does not align with the GDM cohort."
            )
    return co_outcomes
...
def summarize_co_outcome_enrichment(trajectory_df, co_outcomes):
    ...
    group_masks = {
        "Whole GDM modelling cohort": pd.Series(True, index=trajectory_df.index),
...
def print_trajectory_analysis(trajectory_df, confidence, co_outcomes=None):
    print("\n" + "=" * 100)
    print(
        f"SUBJECT-LEVEL GDM TRAJECTORY ANALYSIS: confidence={confidence:.0%}, "
        f"alpha={1.0-confidence:.2f}"
    )
    print("=" * 100)
    eligible = trajectory_df.loc[trajectory_df["U3_landmark_eligible"]]
    print(
        f"Master cohort N={len(trajectory_df)}, GDM events={int(trajectory_df['GDM'].sum())} "
        f"({trajectory_df['GDM'].mean():.2%}); U3 risk set N={len(eligible)}, "
        f"events={int(eligible['GDM'].sum())} ({eligible['GDM'].mean():.2%}); "
    ...
    print("\n4) Exact trajectory distribution among GDM-positive pregnancies in the U3 risk set:")
    ...
    print("\n5) Clinically grouped trajectories and observed GDM incidence:")
    print(grouped_summary.to_string(index=False, float_format=lambda x: f"{x:.4f}"))

    transition_tables = {}
    for positive_only in [False, True]:
        subset_label = "GDM-positive only" if positive_only else "all samples"
        for from_visit, to_visit in [("U1", "U2"), ("U2", "U3")]:
...
OUTCOME_NAME_FOR_ALIGNMENT = "GDM"


def _full_cohort_exclusion_reason():
    odm = np.asarray(odm_raw, dtype=float)
    reason = np.full(FULL_COHORT_N, "", dtype=object)
    reason[~np.isfinite(odm)] = "missing_oDM"
    reason[np.isfinite(odm) & (odm == 1.0)] = "pre_existing_DM"
    unsupported = np.isfinite(odm) & ~np.isin(odm, [1.0, 2.0, 3.0])
    reason[unsupported] = "unsupported_oDM_code"
    return reason
...
def save_analysis_results(
    ...
    excel_path = os.path.join(OUTPUT_DIR, f"GDM_{MODEL_SELECTION}_CP_results.xlsx")
    ...
    with open(config_path, "w", encoding="utf-8") as handle:
        handle.write("Outcome: GDM (oDM=2 versus oDM=3)\n")
        handle.write("Positive class: oDM=2 (GDM)\n")
        handle.write("Negative class: oDM=3 (non-GDM)\n")
        handle.write(
            f"Excluded oDM=1 pre-existing DM: {GDM_DM_EXCLUDED_N}\n"
        )
        handle.write(
            f"Excluded missing oDM: {GDM_MISSING_EXCLUDED_N}\n"
        )
        handle.write(
            f"Excluded unsupported oDM codes: {GDM_OTHER_EXCLUDED_N}\n"
        )
        handle.write(
            "Target-derived GDM and pre-existed DM variables excluded from all feature sets\n"
        )
...
def run_performance_visit(visit_name, feature_names, cv_splits, analysis_mask):
    ...
    summary = {
        "outcome": "GDM", "visit": visit_name, "n": int(len(y)), "events": int(y.sum()),
...
summary_path = os.path.join(OUTPUT_DIR, "GDM_nestedCV_performance_summary.csv")
subject_path = os.path.join(OUTPUT_DIR, "GDM_nestedCV_subject_level_oof_predictions.csv")
tuning_path = os.path.join(OUTPUT_DIR, "GDM_nestedCV_inner_tuning_all_candidates.csv")
selected_path = os.path.join(OUTPUT_DIR, "GDM_nestedCV_selected_hyperparameters_by_fold.csv")
